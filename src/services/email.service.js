...
export const sendOtpEmail = async (email, otp) => {
  const mailOptions = {
    from: `"EasyToFindEdu" <${env.smtp.user}>`,
    to: email,
    subject: "Your Verification Code - EasyToFindEdu",
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto; padding: 20px;">
        <h2 style="color: #2563eb; text-align: center;">EasyToFindEdu</h2>
        <p>Hello,</p>
        ...
        <p style="color: #6b7280; font-size: 12px; text-align: center;">EasyToFindEdu - Education Platform</p>
      </div>
    `,
  };
...
export const sendEnquiryEmailToOwner = async ({ ownerEmail, ownerName, enquiry, batch, course, institute }) => {
  ...
  const mailOptions = {
    from: `"EasyToFindEdu" <${env.smtp.user}>`,
    to: ownerEmail,
    subject: `New Enquiry for ${batch.batchName} - EasyToFindEdu`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; background: #f9fafb; border-radius: 12px;">
        ...
        <div style="background: #2563eb; padding: 20px 24px; border-radius: 8px 8px 0 0; text-align: center;">
          ...
          <p style="color: #bfdbfe; margin: 6px 0 0; font-size: 14px;">via EasyToFindEdu</p>
        </div>
        ...
        <p style="color: #9ca3af; font-size: 11px; text-align: center; margin: 16px 0 0;">
          This enquiry was submitted via EasyToFindEdu. Do not reply to this email directly.
        </p>
      </div>
    `,
  };
...
export const sendEnquiryConfirmationToStudent = async ({ studentEmail, studentName, batch, institute, course }) => {
  const mailOptions = {
    from: `"EasyToFindEdu" <${env.smtp.user}>`,
    to: studentEmail,
    subject: `Enquiry Submitted for ${batch.batchName} - EasyToFindEdu`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto; padding: 20px;">
        <h2 style="color: #2563eb; text-align: center;">EasyToFindEdu</h2>
        ...
        <p style="color: #6b7280; font-size: 13px;">If you have further questions, you can submit another enquiry from the EasyToFindEdu app.</p>
        <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 20px 0;">
        <p style="color: #6b7280; font-size: 12px; text-align: center;">EasyToFindEdu - Education Platform</p>
      </div>
    `,
