...
export const register = asyncHandler(async (req, res) => {
  const { name, email, phone, password, gender, lastQualification, referralCode } = req.body;
  const result = await studentAuthService.registerStudent(
    name, email, phone, password, gender, lastQualification, referralCode
  );
...
export const adminUpdateStudentCoins = asyncHandler(async (req, res) => {
  const { coins } = req.body;
  const studentId = req.params.id;

  if (typeof coins !== 'number') {
    throw new ApiError(400, "Coins must be a number.");
  }

  const student = await studentAuthService.adminUpdateStudentCoins(studentId, coins);
  return res.status(200).json(new ApiResponse(200, "Student coins updated successfully.", student));
});
