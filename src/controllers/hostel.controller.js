 ...
 * @throws {ApiError} 404 - Hostel not found
 */
export const wardenInquiry = asyncHandler(async (req, res) => {
  const { id } = req.params;
  ...
  const hostel = await Hostel.findById(id).select("_id name").lean();
  if (!hostel) {
  ...
  const inquiry = await HostelInquiry.create({
    ...
    phone,
    email,
    address: purpose ?? "Warden contact inquiry",
    profession: "student",
    hostel: id,
  });

  res.status(201).json(
    new ApiResponse(201, "Inquiry submitted successfully.", { inquiryId: inquiry._id })
  );
});
