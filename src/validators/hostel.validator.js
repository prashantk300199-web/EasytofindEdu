...
export const createHostelSchema = Joi.object({
  ...
  washroom_amenities: Joi.alternatives()
    .try(
      Joi.array().items(Joi.string().valid(...AMENITY_KEYS)),
      Joi.string().custom(parseJSON)
    )
  ...
  building_amenities: Joi.alternatives()
    .try(
      Joi.array().items(Joi.string().valid(...AMENITY_KEYS)),
      Joi.string().custom(parseJSON)
    )
  ...
  security: Joi.alternatives()
    .try(
      Joi.object({
        ...
        fire_extinguisher: joiBoolean.default(false),
        transport_facilities: joiBoolean.default(false),
      }),
      Joi.string().custom(parseJSON)
    )
    .default({}),
  ...
  legal_docs: Joi.alternatives().try(
    Joi.object({
      ...
      member_of_hostel_wellfare_association: joiBoolean.default(false),
    }),
    Joi.string().custom(parseJSON)
  ).default({}),
  ...
  /* --------------------------- Contact Info ----------------------------- */
  contact_info: Joi.alternatives().try(
    Joi.object({
      phone: Joi.string().trim().allow("").empty(""),
